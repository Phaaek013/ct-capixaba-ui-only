...
// PATCH /api/coach/modelos-treino/[id] - Atualizar modelo
export async function PATCH(
  ...
  try {
    ...
    // Verifica se o modelo existe e pertence ao coach
    const existente = await prisma.treino.findFirst({
      where: { id: modeloId, coachId, ehModelo: true },
    });
    ...
    // Verifica se já existe outro modelo com esse nome (do mesmo coach)
    const duplicado = await prisma.treino.findFirst({
      where: {
        coachId,
        ehModelo: true,
    ...
    if (duplicado) {
      return NextResponse.json(
        { error: "Já existe outro modelo com esse título" },
        { status: 400 }
      );
    }
...
// DELETE /api/coach/modelos-treino/[id]
